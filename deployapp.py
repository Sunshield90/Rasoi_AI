...
def show_login_page():
    st.markdown("""
        <style>
            ...
            /* 5. TYPOGRAPHY */
            h1 { font-size: 4rem !important; font-weight: 900 !important; text-shadow: 0 4px 10px black; }
            .highlight { color: #FFA726; text-shadow: 0 0 20px rgba(255,167,38,0.6); }
    ...
    with c1:
        ...
        st.markdown("# Master your kitchen with <span class='highlight'>Rasoi AI</span>.", unsafe_allow_html=True)
...
# --- 3. MAIN APP (History Logic Added) ---
def show_main_app():
    try:
    ...
    st.markdown("""
        <style>
            ...
            h1, h2, h3, h4 { color: #FFA726 !important; text-shadow: none !important; }
            .stCheckbox label { color: #FAFAFA !important; font-size: 1rem; }
        </style>
    """, unsafe_allow_html=True)

    if 'theme_color' not in st.session_state: st.session_state['theme_color'] = "#FFA726"
    ...
    with col_right:
        ...
        st.markdown(f"<h3 style='margin-top:0;'>🍳 Create your plate</h3>", unsafe_allow_html=True)
        ...
        with c4: occasion = st.selectbox("Occasion", ["Daily Meal", "Pre-Workout 💪", "Post-Workout ⚡", "Hangover Cure 🤕", "Date Night 🍷", "Sick Day 🍵", "Festival 🎉", "Midnight Craving 🌙", "Tiffin/Lunchbox 🍱", "Birthday Party 🎂", "Potluck 🍲", "Picnic 🧺", "Brunch 🥞", "High Tea ☕", "Road Trip 🚗"])
                ...
                with container:
                    with st.spinner(f"👨‍🍳 Chef is planning your {occasion}..."):
                        try:
                            ...
                            # --- HISTORY SAVING LOGIC (ADDED HERE) ---
                            if 'history' not in st.session_state: st.session_state['history'] = []
            ...
            with c_y:
                if st.button("🤢 Continue (Weird Dessert)"):
                    ...
                    with st.spinner("Processing..."):
                        ...
                        # Save History here too
                        if 'history' not in st.session_state: st.session_state['history'] = []
                        st.session_state['history'].insert(0, data.get('title'))
    ...
    if 'recipe_data' in st.session_state and not st.session_state.get('conflict_active'):
        ...
        with container:
            ...
            with c_title:
                st.markdown(f"<h1 style='color:#FFA726; margin-bottom:0;'>{data.get('title')}</h1>", unsafe_allow_html=True)
            ...
            with c_notes:
                ...
                st.markdown(f"<div style='background:rgba(255,255,255,0.05); padding:15px; border-radius:12px; border-left:4px solid #FFA726;'><h4 style='color:#FFA726; margin:0;'>👅 Chef's Notes</h4><p style='color:#EEE; font-style:italic;'>\"{data.get('tasting_notes')}\"</p><div style='color:#4CAF50; font-weight:bold; margin-top:10px;'>PROFILE: {data.get('taste_profile', 'SAVORY').upper()}</div></div>", unsafe_allow_html=True)
            ...
            for i in range(4):
                if i < len(bevs):
                    d = bevs[i]
                    with cols[i]: st.markdown(f"<div style='background:rgba(255,255,255,0.05); padding:15px; border-radius:12px;'><div class='som-title'>{d.get('name')}</div><span style='font-size:0.75rem; color:#AAA;'>{d.get('type')}</span><hr style='border-color:rgba(255,255,255,0.1);'><div style='font-size:0.85rem; color:#DDD;'>🥣 {d.get('quick_recipe')}</div></div>", unsafe_allow_html=True)
            ...
            with c_steps:
                ...
                for i, step in enumerate(data.get('instructions', [])):
                    st.markdown(f"<div style='margin-bottom:25px;'><div style='color:#FFA726; font-weight:bold;'>Step {i+1}</div><div style='background:rgba(255,255,255,0.05); padding:15px; border-radius:8px;'>{step['text']}</div></div>", unsafe_allow_html=True)
